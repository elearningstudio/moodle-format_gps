...
class format_gps extends format_base {
    ...
    public function create_edit_form_elements(&$mform, $forsection = true) {
        ...
        if ($validationerror == 'yes') {
            $error = html_writer::tag('div', new lang_string('validationerror', 'format_gps'),
                    array('class' => 'bold red error'));
            $errorlabel = html_writer::tag('div', new lang_string('error'), array('class' => 'bold red error'));
